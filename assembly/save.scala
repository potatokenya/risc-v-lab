...
class save(file: String) extends Module {
  ...
  pcNext := Mux(exmem.jumpTaken, exmem.jumpTarget, Mux(exmem.branchTaken, exmem.branchTarget, pc + 4.U))

  when(exmem.branchTaken || exmem.jumpTaken) {
    ifid := 0.U.asTypeOf(new IFID)  // flush on taken branch/jump
  }.elsewhen(!stall) {
    pc := pcNext
    ifid.pc := pc
    ifid.instr := instr
  ...
  // B-type immediate (Branches), properly sign-extended to 32 bits
  /* val imm_b = Cat(Fill(19, ifid.instr(31)),  // sign extend
     ifid.instr(31),           // imm[12]
     ifid.instr(7),            // imm[11]
     ifid.instr(30,25),        // imm[10:5]
     ifid.instr(11,8),         // imm[4:1]
     0.U(1.W)                  // imm[0]
   )
   */
  // Extract individual fields
  val b_imm_12   = ifid.instr(31)
  val b_imm_11   = ifid.instr(7)
  val b_imm_10_5 = ifid.instr(30,25)
  val b_imm_4_1  = ifid.instr(11,8)

  // Combine into a 13-bit signed number
  val imm_b13 = Cat(b_imm_12, b_imm_11, b_imm_10_5, b_imm_4_1, 0.U(1.W)).asSInt

  // Sign-extend to 32 bits
  val imm_b = imm_b13.asSInt.asUInt
  ...
  when(exmem.branchTaken || exmem.jumpTaken) {
    ...
    idex.imm := MuxLookup(opcode, imm_i, Seq(
      "b0100011".U -> imm_s, // Store
      "b1100011".U -> imm_b  // Branch
    ))
    ...
    idex.jump     := ctrl.io.jump
    idex.jumpReg  := ctrl.io.jumpReg
    idex.wbSelPC4 := ctrl.io.wbSelPC4
  }
  /*
  ...
  // Combine branch signal and condition
  ...
  // JAL jump
  val pcPlus4 = idex.pc + 4.U

  val jalTarget  = idex.pc + idex.imm
  val jalrTarget = (forwardA + idex.imm) & "hFFFFFFFE".U
  ...
  //printf(p"STORE addr=${exmem.aluResult} STORE data=${exmem.regData2}\n")
  ...
  /*
  printf(p"wbData: memWbReg.memOut=${memwb.memData} memWbReg.aluOut=${memwb.aluResult}\n")
  printf(p"exmem rd=${exmem.rd}  memWB rd=${memwb.rd} data=${wbData} we=${memwb.regWrite}\n")
