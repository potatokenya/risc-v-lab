...
class Top(file: String) extends Module {
  ...
  // Peripherals
  // =========================================================
  val mmUart     = Module(new MemoryMappedUart(100000000, 9600, 8, 8))
  ...
  // Peripheral routing
  // =========================================================
  mmUart.io.port.read   := mmRead && uartSel
  mmUart.io.port.write  := mmWrite && uartSel
  mmUart.io.port.addr   := addr
  mmUart.io.port.wrData := wrData

  mmLeds.io.port.read   := mmRead && ledsSel
  mmLeds.io.port.write  := mmWrite && ledsSel
  mmLeds.io.port.addr   := addr
  mmLeds.io.port.wrData := wrData

  mmSevenSeg.io.port.read   := mmRead && segSel
  mmSevenSeg.io.port.write  := mmWrite && segSel
  ...
  // MMIO readback
  // =========================================================
  val mmioReadData = Mux(uartSel, mmUart.io.port.rdData,
    Mux(ledsSel, mmLeds.io.port.rdData,
